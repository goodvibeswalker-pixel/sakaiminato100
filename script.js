...
const themeDetails = {
  ...
  healthSports: {
    ...
    healthArticle: {
      ...
      triathlon: [
        ["全日本トライアスロン皆生大会", "1981年に始まった国内ロングトライアスロンの歴史ある大会。皆生温泉を中心に、鳥取県西部の海、山、まちをつなぐスポーツイベントとして見られる。"],
        ["境港市との関わり", "2026年大会のランは、皆生プレイパークから境港市方面を通り、どらドラパーク米子陸上競技場へ向かうコース。境港は応援、交通、安全、景観で大会を支えるエリアになる。"],
        ["健康づくりへの広げ方", "大会そのものは上級者向けでも、住民にはランニング、ウォーキング、自転車、スイムを小さく分けて取り入れる入口になる。観戦やボランティアも健康参加の一つ。"],
        ["注意したい点", "大会日は交通規制や迂回、暑熱対策、給水、救護、海・道路の安全確認が重要。観光客にも住民にも分かりやすい案内が必要になる。"],
      ],
      insight: [
        ...
        "トライアスロンは、境港市にとって競技者だけのイベントではありません。ランコースや応援場所をきっかけに、普段のウォーキング、自転車利用、海辺の景観、地域ボランティア、防災・救護の意識までつなげられます。",
      ...
      sources: [
        ...
        ["全日本トライアスロン皆生大会", "第44回大会の公式情報、コースガイド、大会当日の案内を確認。"],
        ["Triathlon Lumina 全日本トライアスロン皆生 2026", "開催地、競技距離、境港市を含むランコースの概要を確認。"],
      ],
    },
    sections: [
...
function renderHealthArticle(article) {
  ...
  const triathlon = article.triathlon
    .map(
      ([title, text]) => `
        <article class="health-triathlon-card">
          <h5>${title}</h5>
          <p>${text}</p>
        </article>
      `,
  ...
  return `
    <div class="health-article">
      ...
      <section class="health-panel health-triathlon-panel">
        <div class="panel-heading">
          <p class="section-kicker">Triathlon</p>
          <h4>トライアスロンを地域の健康資源として見る</h4>
          <p>競技者だけでなく、応援、ボランティア、交通安全、景観づくりまで含めて、境港らしいスポーツ文化として整理します。</p>
        </div>
        <div class="health-triathlon-grid">${triathlon}</div>
      </section>
